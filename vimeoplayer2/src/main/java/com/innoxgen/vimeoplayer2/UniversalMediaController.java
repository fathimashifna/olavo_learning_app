...
package com.innoxgen.vimeoplayer2;
...
public class UniversalMediaController extends FrameLayout {
    ...
    private View mBackButton;// Back button
    ...
    private void initControllerView(View v) {
        ...
        if (mCenterPlayButton != null) {//Start playing again

            mCenterPlayButton.setOnClickListener(mCenterPlayListener);
        }

        if (mBackButton != null) {//The return button is only visible in full screen state
    ...
    /**
     * Use to set video view to full screen
     * @param isFullscreen
     */
    public void setmIsFullScreen(boolean isFullscreen) {
        if (isFullscreen) {
            mScaleButton.setVisibility(VISIBLE);
        } else mScaleButton.setVisibility(GONE);
     ...
     */
    public void show(int timeout) {//Only responsible for the display of the upper and lower bars, not responsible for the central loading, error, playBtn display.
        if (!mShowing) {
            setProgress();
            if (mTurnButton != null) {
                mTurnButton.requestFocus();
    ...
    public void hide() {//Only responsible for the display of the upper and lower bars, not responsible for the central loading, error, playBtn display.
        if (mShowing) {
            mHandler.removeMessages(SHOW_PROGRESS);
...
//        super.setEnabled(enabled);
        ...
        mBackButton.setEnabled(true);//The return key in the upper right corner of the full screen is always available.
    }
...
//    }
    ...
    public interface MediaPlayerControl {
        ...
        void closePlayer();//Turn off playing the video and make the player in idle state
